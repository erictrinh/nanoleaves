...
class Aurora {
	constructor(opts) {
		...
		this.apibase = `http://${opts.host}:${opts.port}/api/v1`;

		this.req = axios.create({
			baseURL: `http://${opts.host}:${opts.port}/api/v1/${opts.token}`,
		});
	}
	...
	state(field) {
		return this.req.get(`/state/${field}`).then((rez) => {
			if (rez.data.hasOwnProperty('value')) return rez.data.value;
			return rez.data;
		});
	}
	...
	effects() {
		return this.req.get('/effects/effectsList').then((rez) => rez.data);
	}
	...
	setStaticPanel(input) {
		...
		const tweaked = input.map((p) => {
			if (p instanceof Panel) return p;
